...
namespace EntityProvider
{
    public partial class DataAccess
    {
        ...
        {
            var dbModel = await CreateOrganizationItem(_context, model);
            await _context.SaveChangesAsync();
            model.Id = dbModel.Id;
            return model.Id;
        }
        private async Task<OrganizationItem> CreateOrganizationItem(CharityContext _context, OrganizationItemModel model)
        {
            ...
            {
                var currentItems = await GetCurrentOrganizationItems(_context, model.Organization.Id);
        ...
        public async Task<bool> UpdateOrganizationItem(OrganizationItemModel model)
        {
            ...
            if (IsOrganizationMemberModerator(memberOrgRoles))
            {
                OrganizationItem dbModel = await _context.OrganizationItems.Where(x => x.Id == model.Id && x.IsDeleted == false).FirstOrDefaultAsync();
                if (dbModel != null)
                {
        ...
        public async Task<bool> CreateMultipleOrganizationItem(List<OrganizationItemModel> organizationItems)
        {
            ...
            if (IsOrganizationMemberModerator(memberOrgRoles))
            {
                var currentItems = await GetCurrentOrganizationItems(_context, organizationId);
            ...
            if (IsOrganizationMemberModerator(memberOrgRoles))
            {
                using (var transaction = _context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            OrganizationItem dbModel = await _context.OrganizationItems.Where(x => x.Id == orgItemModel.Id && x.IsDeleted == false).FirstOrDefaultAsync();
                            if (dbModel != null)
                            {
        ...
        public async Task<bool> DeleteOrganizationItems(List<int> ids)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        OrganizationItem dbModel = await _context.OrganizationItems.Where(x => x.Id == id && x.IsDeleted == false).FirstOrDefaultAsync();
        ...
        public async Task<bool> DeleteOrganizationItem(int organizationId, int itemId)
        {
            ...
            if (IsOrganizationMemberModerator(memberOrgRoles))
            {
                OrganizationItem dbModel = await _context.OrganizationItems.Where(x => x.OrganizationId == organizationId && x.ItemId == itemId && x.IsDeleted == false).FirstOrDefaultAsync();
                if (dbModel != null)
                {
                    dbModel.IsDeleted = true;
                    return await _context.SaveChangesAsync() > 0;
        ...
        {
            return await (from oi in _context.OrganizationItems
                          join o in _context.Organizations on oi.OrganizationId equals o.Id
                          join i in _context.Items on oi.ItemId equals i.Id
                          join c in _context.Campaigns on oi.CampaignId equals c.Id into tc
                          from c in tc.DefaultIfEmpty()
                          join ciuom in _context.Uoms on oi.CampaignItemUom equals ciuom.Id into tci
        ...
        {
            var orgItemQueryable = (from oi in _context.OrganizationItems
                                    join o in _context.Organizations on oi.OrganizationId equals o.Id
                                    join i in _context.Items on oi.ItemId equals i.Id
        ...
        private async Task<List<OrganizationItem>> GetCurrentOrganizationItems(CharityContext _context, int organizationId)
        {
            return await _context.OrganizationItems.Where(x => x.OrganizationId == organizationId && (x.CampaignId == null || x.CampaignId == 0) && x.IsDeleted == false).ToListAsync();
        }
